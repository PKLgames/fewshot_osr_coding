...
class TAUDataset:
    def __init__(
        ...
        """
        TAU-urban-acoustic-scenes-2019-development 数据集的 Dataset 类，
        ...
        if split in {"train","train_eval"}:
            dataset_path = "/coding/TAU-urban-acoustic-scenes-2019-development/sampled_setup/sampled_fold1_train.csv"
            self.save_file_path = '/coding/audio_classifying/unprocessed_data/TAU19_train_pro_' + f'{max_duration}' + 's.pt'
        elif split == "calib":
            dataset_path = "/coding/TAU-urban-acoustic-scenes-2019-development/sampled_setup/sampled_fold1_calib.csv"
            self.save_file_path = '/coding/audio_classifying/unprocessed_data/TAU19_calib_pro_' + f'{max_duration}' + 's.pt'
        elif split=="test":
            dataset_path = "/coding/TAU-urban-acoustic-scenes-2019-development/sampled_setup/sampled_fold1_evaluate.csv"
            self.save_file_path = '/coding/audio_classifying/unprocessed_data/TAU19_eval_pro_' + f'{max_duration}' + 's.pt'

        self.dataset_map = self.csv_to_dataset(dataset_path)
        # 进度检测
        self.dataset_map_len = len(self.dataset_map)
        self.max_duration = max_duration
        self.dataset_dir = "/coding/TAU-urban-acoustic-scenes-2019-development"
        # self.data_type = ".wav"
        self.saving_audio_batch = 64
        # 读取 label_mapping 字典
        label_mapping_csv = '/coding/TAU-urban-acoustic-scenes-2019-development/sampled_setup/vocabulary.csv'
    ...
    def __getitem__(self, index):
        if self.use_cache:
            ...
            # 确保音频为单声道（mono）：如果音频是多维的（如立体声），取平均转为单声道
            if audio.dim() > 1:
