...
class TFYamn():
    ...
    def yamnet_tf_model():
        ...
        spectrogram, patches = features_lib.waveform_to_log_mel_spectrogram_patches(
              tf.squeeze(waveform, axis=0), tf_params)
        predictions, embeddings = yamnet(patches, tf_params)
        # only difference is here
        frames_model = Model(
            name='yamnet_frames',
            inputs=waveform,
            outputs=[spectrogram, patches, predictions, embeddings]
        )
        return frames_model
...
def main():
    ...
    snd = snd.reshape(1, -1)
    spec, patches, tf_pred, tf_emb = tf_model(snd)
    ...
    assert np.allclose(pt_pred, tf_pred, atol=6e-4)
