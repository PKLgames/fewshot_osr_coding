...
class AffineFlow:
    ...
    def __init__(self, input_dim=2):
        ...
        self.log_det = np.log(np.abs(np.linalg.det(self.A)))
        
    def forward(self, h):
        """正向变换: z = A * h + b, 使得z服从标准正态分布"""
        z = h @ self.A.T + self.b
        return z, self.log_det
    
    def inverse(self, z):
        """逆向变换: h = A^{-1} * (z - b)"""
        h = (z - self.b) @ self.A_inv.T
        return h
...
def pf_qf_k(h, flows, pi, standard_normal):
    ...
    for m, flow in enumerate(flows):
        ...
        z, log_det = flow.forward(h)
        det_value = np.exp(log_det)  # 雅可比行列式的绝对值
        
        # 注意：我们需要对每个点使用其对应的det_value
        # 将det_value从(n_points,)变为(n_points, 1)以便广播
        # det_value_expanded = det_value[:, np.newaxis]  # 错误：det_value是标量
        
        # qf_km(h) = f_km(h) * |det(J_{f_km}(h))| = z * exp(log_det)
        qf_values[m] = z * det_value
...
# 用颜色表示pf_k(h)的模长
pf_contour = ax5.contourf(X, Y, pf_magnitude, levels=50, cmap='plasma', alpha=0.7)
# 叠加概率密度等高线
density_contour = ax5.contour(X, Y, Z, levels=10, colors='white', linewidths=1.5, alpha=0.8)
ax5.clabel(density_contour, inline=True, fontsize=8, colors='white')
...
ax5.legend()
fig.colorbar(pf_contour, ax=ax5, label='pf_k(h)映射值模长')
...
# 绘制加权平均qf映射值
qf_contour = ax6.contourf(X, Y, weighted_qf, levels=50, cmap='coolwarm', alpha=0.7)
# 叠加概率密度等高线
...
ax6.legend()
fig.colorbar(qf_contour, ax=ax6, label='加权平均qf_km(h)映射值模长')
...
sample_grid = grid_points[sample_indices]
sample_density = Z.flatten()[sample_indices]
sample_pf_mag = pf_magnitude.flatten()[sample_indices]

scatter = ax5.scatter(sample_grid[:, 0], sample_grid[:, 1], 
                     c=sample_pf_mag, s=10, cmap='plasma', 
...
# 创建3D散点图
ax6_3d = fig3.add_subplot(236, projection='3d')
scatter3d = ax6_3d.scatter(sample_points[:, 0], sample_points[:, 1], sample_pf_mag,
                       c=sample_density, s=20, cmap='viridis', alpha=0.7, 
                       edgecolor='black', linewidth=0.3)
ax6_3d.set_xlabel('X', fontsize=12)
ax6_3d.set_ylabel('Y', fontsize=12)
ax6_3d.set_zlabel('pf_k(h) 模长', fontsize=12)
ax6_3d.set_title('概率密度与映射值模长的3D关系', fontsize=14)
fig3.colorbar(scatter3d, ax=ax6_3d, label='概率密度 p_k(h)')
...
print("\n=== 概率密度验证 ===")
# 随机选择一些点，验证概率密度是否正确
test_points = np.array([[0, 0], [1, 1], [2, -1], [-2, 2]])
for i, point in enumerate(test_points):
    point = point.reshape(1, -1)
    density = p_k(point, flows, pi, standard_normal)[0]
    
    # 验证概率密度是非负的
    assert density >= 0, f"点{point}的概率密度为负: {density}"
    
    # 验证通过逆变换采样得到的点的概率密度
    if i == 0:
        # 对第一个点，从每个混合成分采样并计算概率密度
        for m in range(num_components):
            z = np.random.multivariate_normal(np.zeros(input_dim), np.eye(input_dim))
            h_sampled = flows[m].inverse(z.reshape(1, -1))
            density_sampled = p_k(h_sampled, flows, pi, standard_normal)[0]
            print(f"  从成分{m+1}采样的点 {h_sampled[0]} 的概率密度: {density_sampled:.6f}")
    
    print(f"  点 {point[0]} 的概率密度: {density:.6f}")

# 14. 验证变换后的z是否服从标准正态分布
print("\n=== 变换验证 ===")
# 随机生成一些h点
test_h = np.random.randn(1000, 2)
for m, flow in enumerate(flows):
    z_transformed, _ = flow.forward(test_h)
    
    # 计算z的均值和协方差
    z_mean = np.mean(z_transformed, axis=0)
    z_cov = np.cov(z_transformed, rowvar=False)
    
    print(f"成分 {m+1} 变换后的z统计:")
    print(f"  均值: {z_mean}")
    print(f"  协方差对角线: {np.diag(z_cov)}")
    print(f"  (理论上应为均值[0,0]，协方差[[1,0],[0,1]])")